...
@optionalTypeArgs TResult when<TResult extends Object?>({required TResult Function( String message)  network,required TResult Function( String message)  server,required TResult Function( String message)  cache,required TResult Function( String message)  validation,required TResult Function( String message)  notFound,required TResult Function( String message)  unknown,}) {final _that = this;
switch (_that) {
case NetworkFailure():
...
class NetworkFailure implements Failure {
  const NetworkFailure([this.message = 'Ошибка сети. Проверьте подключение к интернету']);
...
class ServerFailure implements Failure {
  const ServerFailure([this.message = 'Ошибка сервера. Попробуйте позже']);
...
class CacheFailure implements Failure {
  const CacheFailure([this.message = 'Ошибка локального хранилища']);
...
class ValidationFailure implements Failure {
  const ValidationFailure([this.message = 'Некорректные данные']);
...
class NotFoundFailure implements Failure {
  const NotFoundFailure([this.message = 'Данные не найдены']);
...
class UnknownFailure implements Failure {
  const UnknownFailure([this.message = 'Неизвестная ошибка']);
